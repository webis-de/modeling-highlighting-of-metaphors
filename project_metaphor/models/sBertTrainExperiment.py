...
model_name = './model_config/hghl_new_setup/distilbert-base-uncased-single-hghl-split0-batch32-new-2023-02-19_10-52-17/'
# model_name = './model_config/scm_new_setup/distilbert-base-uncased-single-hghl-split0-batch32-new-2023-02-19_12-13-47/'
...
if add_xIntent == 1:
    wandb.init(project=model_name+"-"+task+"-"+sub_task+"-split-"+str(test_split)+"-eval_sub- "+eval_subset+"-with_csr-batch_size- "+str(train_batch_size)+"-again-sat", entity="msen")
else:
    # wandb.init(project="another_just_dbert"+"-"+task+"-"+sub_task+"-split-"+str(test_split)+"-eval_sub- "+eval_subset+"-without_csr-batch_size- "+str(train_batch_size)+"-testwOUThghlasfeat", entity="msen")
    wandb.init(project="scm_new_pretrainedhghl", entity="msen")
...
def train_model_single_task(itr,
            ...
            if loss=='MultipleNegativesRankingLoss':
                ...
                if task=='hghl':
                    if add_xIntent == 0:
                        ...
                        print('*** Adding knowledge to train data ***')
                        train_examples.append(InputExample(texts=[
                            row['Sentence']+'<SEP>'+
    ...
    if task=='hghl':
        if add_xIntent == 0:
            ...
            print('*** Adding knowledge to dev data ***')
            sentences = var['Sentence']+'<SEP>'+ \
                    var['Clean CSR_xWant']+'<SEP>'+ \
                    var['Clean CSR_xIntent']+'<SEP>'+ \
                    var['Clean CSR_xAttr']+'<SEP>'+ \
                    var['Clean CSR_xReact']+'<SEP>'+ \
                    var['Clean CSR_xEffect']+'<SEP>'+ \
                    var['Clean CSR_xNeed']+'<SEP>'+ \
                    var['Clean CSR_xReason']
            pos = var['Schema Slot']
    
    else:
        if add_xIntent == 0:
            print('INTENDED TEST BLOCK')
            sentences = var['Sentence']
            pos = var['Source CM']
        else:
            print('*** Adding knowledge to dev/test data ***')
            sentences = var['Sentence']+'<SEP>'+ \
                    var['Clean CSR_xWant']+'<SEP>'+ \
                    var['Clean CSR_xIntent']+'<SEP>'+ \
                    var['Clean CSR_xAttr']+'<SEP>'+ \
...
def train_model_multitask(itr,
        ...
        for rowx in reader1:
            if loss=='MultipleNegativesRankingLoss':
                if add_xIntent == 0:
                    ...
                    rowx['Sentence']+'<SEP>'+
                    rowx['Source LM']+'<SEP>'+
                    rowx['Clean CSR_xWant']+'<SEP>'+
                    rowx['Clean CSR_xIntent']+'<SEP>'+
                    rowx['Clean CSR_xAttr']+'<SEP>'+
                    rowx['Clean CSR_xReact']+'<SEP>'+
                    rowx['Clean CSR_xEffect']+'<SEP>'+
                    rowx['Clean CSR_xNeed']+'<SEP>'+
                    rowx['Clean CSR_xReason'], 
                    rowx['Schema Slot']]))
    ...
    if task=='hghl':
        if add_xIntent == 0:
            sentences_hghl = var_hghl['Sentence'] + '<SEP>' + \
                        var_hghl['Source LM']
            pos_hghl = var_hghl['Schema Slot']
        else:
            print('*** Adding knowledge to dev data ***')
            sentences_hghl = var_hghl['Sentence']+'<SEP>'+ \
                    var_hghl['Source LM']+'<SEP>'+ \
                    var_hghl['Clean CSR_xWant']+'<SEP>'+ \
                    var_hghl['Clean CSR_xIntent']+'<SEP>'+ \
                    var_hghl['Clean CSR_xAttr']+'<SEP>'+ \
                    var_hghl['Clean CSR_xReact']+'<SEP>'+ \
                    var_hghl['Clean CSR_xEffect']+'<SEP>'+ \
                    var_hghl['Clean CSR_xNeed']+'<SEP>'+ \
                    var_hghl['Clean CSR_xReason']
            pos_hghl = var_hghl['Schema Slot']
            assert len(sentences_hghl) == len(pos_hghl)
    
    else:
        if add_xIntent == 0:
            sentences_scm = var_scm['Sentence'] + '<SEP>' + \
                    var_scm['Source LM']  
            ...
            print('*** Adding knowledge to dev data ***')
            sentences_scm = var_scm['Sentence']+'<SEP>'+ \
                    var_scm['Source LM']+'<SEP>'+ \
                    var_scm['Clean CSR_xWant']+'<SEP>'+ \
                    var_scm['Clean CSR_xIntent']+'<SEP>'+ \
                    var_scm['Clean CSR_xAttr']+'<SEP>'+ \
                    var_scm['Clean CSR_xReact']+'<SEP>'+ \
                    var_scm['Clean CSR_xEffect']+'<SEP>'+ \
                    var_scm['Clean CSR_xNeed']+'<SEP>'+ \
                    var_scm['Clean CSR_xReason']
            pos_scm = var_scm['Source CM']
            assert len(sentences_scm) == len(pos_scm)
...
if task =='single':
    ...
    train_model_single_task(itr,
                ...
                model_suffix='single-hghl-split0-batch32-new', 
                data_file_suffix='', 
                max_seq_length=512, 
                add_special_token=False, 
                loss='MultipleNegativesRankingLoss', 
...
# distilbert-base-uncased-single-hghl-split0-batch32-2022-12-30_14-07-41_best_allx_scm_single_dev
    ...
    train_model_multitask(itr,
                ...
                num_epochs=epochs, 
                train_batch_size=train_batch_size, 
                model_suffix='multi-hghl-split0-batch32', 
                data_file_suffix='', 
                max_seq_length=512, 
                add_special_token=False, 
                loss='MultipleNegativesRankingLoss', 
